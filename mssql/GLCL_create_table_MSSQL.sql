 ...
 *  SQL Server (MSSQL) Database Script
 *
 *  Scope:
 *    - Cruise reservations
...
IF DB_ID(N'GLCL_DB') IS NOT NULL
BEGIN
    ALTER DATABASE GLCL_DB SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE GLCL_DB;
END;
...
CREATE TABLE CruiseOperator (
    ...
    AllowsChaperonedYouth  BIT           NOT NULL DEFAULT 0
);
...
CREATE TABLE CabinCategory (
    CabinCategoryID      INT           IDENTITY(1,1) PRIMARY KEY,
...
CREATE TABLE Cabin (
    ...
    IsWheelchairAccessible BIT           NOT NULL DEFAULT 0,
...
CREATE TABLE RoutePort (
    RoutePortID    INT      IDENTITY(1,1) PRIMARY KEY,
    RouteID        INT      NOT NULL,
    PortID         INT      NOT NULL,
    StopSequence   INT      NOT NULL,
    IsHomePort     BIT      NOT NULL DEFAULT 0,
...
CREATE TABLE CruiseVoyage (
    ...
    VoyageLengthDays      AS (DATEDIFF(day, DepartureDateTime, ArrivalDateTime)) PERSISTED,
...
CREATE TABLE AgeCategory (
    AgeCategoryID  INT          IDENTITY(1,1) PRIMARY KEY,
...
CREATE TABLE BookingCabin (
    BookingCabinID  INT            IDENTITY(1,1) PRIMARY KEY,
 ...
 * DailySupervisionFee is set by the trigger when IsChaperonedYouth = 1.
 */
CREATE TABLE BookingPassenger (
    BookingPassengerID    INT            IDENTITY(1,1) PRIMARY KEY,
    BookingID             INT            NOT NULL,
    ...
    IsChaperonedYouth     BIT            NOT NULL DEFAULT 0,
...
CREATE TABLE DiningOption (
    DiningOptionID  INT           IDENTITY(1,1) PRIMARY KEY,
...
CREATE TABLE ShipDiningOption (
    ShipDiningOptionID  INT  IDENTITY(1,1) PRIMARY KEY,
    ShipID              INT  NOT NULL,
...
CREATE TABLE SpecialtyDiningType (
    SpecialtyDiningTypeID  INT           IDENTITY(1,1) PRIMARY KEY,
...
CREATE TABLE ShipSpecialtyDining (
    ShipSpecialtyDiningID  INT  IDENTITY(1,1) PRIMARY KEY,
    ShipID                 INT  NOT NULL,
...
CREATE TABLE VoyageMealPackageType (
    MealPackageTypeID  INT           IDENTITY(1,1) PRIMARY KEY,
...
CREATE TABLE VoyageMealPackageRule (
    MealPackageRuleID    INT  IDENTITY(1,1) PRIMARY KEY,
...
CREATE TABLE VoyageMealPackage (
    VoyageMealPackageID  INT  IDENTITY(1,1) PRIMARY KEY,
    VoyageID             INT  NOT NULL,
...
CREATE TABLE PassengerSpecialService (
    PassengerServiceID  INT            IDENTITY(1,1) PRIMARY KEY,
...
CREATE TABLE BaggageRule (
    BaggageRuleID  INT            IDENTITY(1,1) PRIMARY KEY,
    OperatorID     INT            NOT NULL,
...
CREATE TABLE BookingBaggage (
    ...
    -- Computed by trigger: 1 if WeightKG exceeds voyage limit
...
CREATE TABLE Excursion (
    ExcursionID    INT            IDENTITY(1,1) PRIMARY KEY,
    PortID         INT            NOT NULL,
...
CREATE TABLE VoyageExcursion (
    VoyageExcursionID  INT  IDENTITY(1,1) PRIMARY KEY,
    VoyageID           INT  NOT NULL,
...
CREATE TABLE BookingExcursion (
    BookingExcursionID  INT            IDENTITY(1,1) PRIMARY KEY,
    BookingPassengerID  INT            NOT NULL,
...
CREATE TABLE BookingCancellation (
    ...
    CancellationDateTime  DATETIME       NOT NULL DEFAULT GETDATE(),
    Reason                VARCHAR(255),
    -- Computed by trigger
...
CREATE TABLE RescheduleRequest (
    RescheduleID       INT             IDENTITY(1,1) PRIMARY KEY,
